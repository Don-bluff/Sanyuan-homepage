    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-bold text-gray-800 font-orbitron">选择扑克牌</h3>
          <button
            onClick={onClose}
        ...
        <div className="space-y-2">
          {/* 表头 - 点数 */}
          <div className="grid grid-cols-13 gap-1 md:gap-2 mb-2">
            {ranks.map(rank => (
              <div key={rank} className="text-center font-bold text-sm p-2 bg-gray-100 rounded">
                {rank}
              </div>
            ))}
          </div>
          ...
          {suits.map(({ suit, symbol, color }) => (
            <div key={suit} className="grid grid-cols-13 gap-1 md:gap-2">
              {ranks.map(rank => {
                ...
                return (
                  <button
                    ...
                    disabled={!canSelect}
                    className={`
                      aspect-[2/3] border-2 rounded-lg text-xs font-bold transition-all duration-200 relative
                      ${selected 
                        ? 'border-blue-500 bg-blue-100 scale-95 shadow-md' 
                        : canSelect 
                          ? 'border-gray-300 bg-white hover:border-gray-500 hover:shadow-md hover:scale-105' 
        ...
        {/* 花色说明 */}
        <div className="flex justify-center gap-4 mt-4 p-3 bg-gray-50 rounded-lg">
          {suits.map(({ suit, symbol, color }) => (
            <div key={suit} className="flex items-center gap-1">
              <span className={`text-lg ${color}`}>{symbol}</span>
              <span className="text-xs text-gray-600 capitalize">{suit}</span>
            </div>
          ))}
        </div>
